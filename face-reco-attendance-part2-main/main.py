...
path = "/home/pi/images/*.jpg*"
for file in glob.glob(path):
    image = cv2.imread(file)
    a=os.path.basename(file)
    b=os.path.splitext(a)[0]
    c=b.split(" ")[1]
    b=b.replace(" {}".format(c),"")
    names.append(b)
    roll.append(c)
    images.append(image)
...
def mysqladddata(names,roll):
       ...
       a = mydb.cursor()
       sql = ("INSERT IGNORE INTO UserAttendance(Date,USERNAME,DATETIME,roll) VALUE(%s,%s,%s,%s)")
       data=(today,names,dtString,roll)
...
while True:
      ...
      for encodeface,facelocation in zip(curframe_encoding,face_locations):
         ...
         rolls=roll[match_index]
         mysqladddata(name,rolls)
        #mysqladddata(rolls)
